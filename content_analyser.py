...
if uploaded_file:
    ...
    end_date = st.date_input("End Date", value=df_filtered['Date'].max())
    ...
    if st.button("Analyze"):
        df_filtered_low_rating = df_filtered[df_filtered['vSp Rating'] <= 3]
        ...
        df_combined = pd.concat([results, df[df['vSp Rating'] > 3]], ignore_index=True)

        justified_low_ratings = results[results['justification'] == 'justified']
        correct_reviews = len(justified_low_ratings) + rating_counts.get(4.0, 0) + rating_counts.get(5.0, 0)
